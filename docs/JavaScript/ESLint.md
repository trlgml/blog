...
> 也可以在全局而不是本地安装 ESLint。但是，你使用的任何插件或可共享配置都必须安装在本地。
```
# 全局安装
...
> extends主要是继承已经配置完成的规则，extends 属性值可以是：
...
## extends和plugins的配合使用（eslint-plugin-react）

1. 安装`npm install eslint-plugin-react --save-dev`

2. 添加 `react` 到plugins中
...
[规则详情](https://eslint.bootcss.com/docs/rules/)
```
// 禁止条件表达式中出现赋值操作符
"no-cond-assign": 2,
